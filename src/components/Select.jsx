...
function Select({ title, options, defaultValue, onChange }) {
  ...
  return (
    <div>
      ...
      <div className='relative flex items-center'>
        <div className='relative w-full cursor-default rounded-lg bg-surfaceAlt/50 hover:bg-surfaceAlt py-2 pl-3 pr-2 text-left appearance-none has-[:focus]:outline-none has-[:focus-visible]:ring-2 has-[:focus-visible]:ring-surfaceAlt2/25 has-[:focus-visible]:ring-offset-2 ring-offset-surface text-md has-[:disabled]:opacity-75 has-[:disabled]:pointer-events-none has-[:disabled]:ring-0 has-[:disabled]:cursor-not-allowed'>
          <select
            defaultValue={defaultValue}
            onChange={handleChange}
            disabled={!hasOptions}
            id={title}
            className='w-full bg-transparent gradient-mask-r-[rgba(0,0,0,1.0)_0%,rgba(0,0,0,1.0)_80%,transparent_90%] appearance-none focus:outline-none'
          >
            {hasOptions ? (
              options.map((option) => {
                return (
